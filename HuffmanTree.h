...
private:
  ...
  void build(std::vector<unsigned> treeCode, std::vector<char> leaves, unsigned* i, char* s, Node* x);
...
void HuffmanTree::build(std::vector<unsigned> treeCode, std::vector<char> leaves, unsigned* i, char* s, Node* x)
{
  if (x == nullptr)
    return;
  else{
